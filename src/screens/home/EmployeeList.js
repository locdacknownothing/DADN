...
export default function EmployeeList({navigation}) {
  ...
  const tempURL = "https://io.adafruit.com/api/v2/Vyvy0812/feeds/pasic-smart-office.temperature/data";
  ...
  const myFunc = () => {
    try {
      fetch(getEmpURL).then(data => data.json()).then(data => {updateEmpName(data[1].name); updateStartWorkTime(data[1].check_in); updateEndWorkTime(data[1].check_out)});
    }
    catch(err){
      console.log(err);
    }
          ...
          <View style = {{flexDirection: 'row'}}>
            ...
            <View style={styles.userInforHolder}>
              ...
              <Text size={12} style={{color: '#8189B0', fontWeight: 'bold'}}>
                  {moment.unix(start_work_time).format("h:mm:ss a")} - {moment.unix(end_work_time).format("h:mm:ss a")}
              </Text>
            </View>
            <View style={{alignSelf: 'flex-end', width: '10%', height: 70, justifyContent: 'center'}}>
              <Icon 
                // Chỗ này cần navigate qua một trang mới nè
              size={30} name='chevron-right' type='feather' color='#878997'></Icon>
            </View>
          </View>
        </View>
      </View>
